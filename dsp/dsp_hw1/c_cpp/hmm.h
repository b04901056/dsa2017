      ...
      cout<<"transition"<<endl;
         ...
         for( i = 0 ; i < hmm->state_num ; i++ )
            for( j = 0 ; j < hmm->state_num ; j++ )
               fscanf(fp, "%.17llf", &( hmm->transition[i][j] ));
      }
       cout<<"observation"<<endl;
         ...
         for( i = 0 ; i < hmm->observ_num ; i++ )
            for( j = 0 ; j < hmm->state_num ; j++ )
               fscanf(fp, "%.17llf", &( hmm->observation[i][j]) );
      } 
   }
}
...
static void dumpHMM( FILE *fp, HMM *hmm )
{
   ...
   for( i = 0 ; i < hmm->state_num - 1; i++ )
      fprintf( fp, "%.17llf ", hmm->initial[i]);
   fprintf(fp, "%.17llf\n", hmm->initial[ hmm->state_num - 1 ] );

   fprintf( fp, "\ntransition: %d\n", hmm->state_num );
   for( i = 0 ; i < hmm->state_num ; i++ ){
      for( j = 0 ; j < hmm->state_num - 1 ; j++ )
         fprintf( fp, "%.17llf ", hmm->transition[i][j] );
   ...
   for( i = 0 ; i < hmm->observ_num ; i++ ){
      for( j = 0 ; j < hmm->state_num - 1 ; j++ )
         fprintf( fp, "%.17llf ", hmm->observation[i][j] );
