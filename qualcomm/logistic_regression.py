...
logisticRegr = LogisticRegression(class_weight = {0 : 1 - weight_positive , 1 : weight_positive})
normalize = {}
...
with open(sys.argv[2],newline='') as csvfile:
    ...
    for i in range(Y.shape[0]):
        if Y[i][0] == 1 and logisticRegr.predict(X[i].reshape(1,-1)) == 1:
            t1_p1 = t1_p1 + 1
        elif Y[i][0] == 1 and logisticRegr.predict(X[i].reshape(1,-1)) == 0:
            t1_p0 = t1_p0 + 1
        elif Y[i][0] == 0 and logisticRegr.predict(X[i].reshape(1,-1)) == 1:
            t0_p1 = t0_p1 + 1
        elif Y[i][0] == 0 and logisticRegr.predict(X[i].reshape(1,-1)) == 0:
            t0_p0 = t0_p0 + 1
